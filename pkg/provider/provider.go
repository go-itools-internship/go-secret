...
func (p *provider) SetData(key, value []byte) error {
	...
	if err != nil {
		return fmt.Errorf("provider, SetData method: error: %w", err)
	}
	saveError := p.dataSaver.SaveData(key, encodedValue)
	if saveError != nil {
		return fmt.Errorf("provider, SetData method: save error: %w", err)
	}
	return nil
}
...
func (p *provider) GetData(key []byte) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("provider, GetData method: error: %w", err)
	}
